...
export default class Crawler {
  ...
  async start() {
    ...
    this.constructor.extendNewNode(this.bootstrapNode)
  ...
  addNode(node) {
    ...
    if (KBucket.distance(node.id, this.nodeId) != 0) {
      this.constructor.extendNewNode(node)
  ...
  static extendNewNode(node) {
    node.lastActive = Date.now()
    if(!node.createdAt) node.createdAt = Date.now()
    node.announceNodes = new Set()
  }

  static calNodeScore(node) {
    return parseInt((Date.now() - node.createdAt) / (1000 * 60)) + node.announceNodes.size * 60
  }
