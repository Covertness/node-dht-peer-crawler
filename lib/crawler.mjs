...
export default class Crawler extends Emitter {
  ...
  getMorePeers() {
    let count = 0
    this.infoHashTable.forEach((info, infoHashStr) => {
      if (count > this.config.maxRouteTableLen || info.announceNodes.size > 0) return

      const infoHash = new Buffer(infoHashStr, 'hex')
      const closestNodes = this.routeTable.closest(infoHash, 16)
      closestNodes.forEach(node => {
        this.getPeers(info, infoHash, node.address)
      })
      count += 1
    })
  }

  getPeers(torrent, infoHash, address) {
  ...
  handleAnnouncePeer(message, address) {
    ...
    if (torrent !== undefined) {
      ...
      torrent.announceNodesAdd(ipStr + ':' + port)
  ...
  addInfoHash(infoHashStr) {
    ...
    torrent.on('announce', (addressStr) => {
      this.emit('announce_peer', infoHashStr, addressStr, torrent)
    })
    torrent.on('destroy', () => {
